  ...
  <main>
        ...
        <div class="buttons">
          <div class="likes">
            <div class="block-button">
              <button class="like" @click="sendLike(1)">
                <i class="far fa-thumbs-up"></i>
              </button>
              <p v-if="areThereLikesInArticle">{{ article.likes.likes }}</p>
              <p v-else>0</p>
            </div>

            <div class="block-button">
              <button class="dislike" @click="sendLike(0)">
                <i class="far fa-thumbs-down"></i>
              </button>
              <p v-if="areThereLikesInArticle">{{ article.likes.dislikes }}</p>
              <p v-else>0</p>
            </div>
          </div>
          <div>
            <button class="comment" id="commentButton" v-on="buttonListener">
              Commenter l'article
            </button>
          </div>
        </div>
        <textarea
          v-if="displayCommentInput"
    ...
    <div v-if="areThereCommentsInArticle">
      ...
      <Comment-display
        ...
        v-for="comment in article.comments.slice().reverse()"
        :key="comment.date"
        :id="comment.id"
...
export default {
  ...
  methods: {
    ...
    async sendComment() {
      ...
      const response = await fetch(this.urlSendComment + id, {
        headers: {
    ...
    async getArticle() {
      ...
      const response = await fetch(this.urlGetArticle + id, {
        headers: {
          Authorization: "Bearer " + token,
          Accept: "application/json",
          "Content-Type": "application/json",
    ...
    async createLike(value) {
      ...
      const response = await fetch(this.urlSendLike + articleId, {
        headers: {
          Authorization: "Bearer " + token,
          Accept: "application/json",
          "Content-Type": "application/json",
    ...
    async deleteLike() {
      ...
      const response = await fetch(this.urlDeleteLike + articleId, {
        headers: {
          Authorization: "Bearer " + token,
          Accept: "application/json",
          "Content-Type": "application/json",
